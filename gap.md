> **Status note — read before planning from this document.**
>
> This backlog was written against `baseline.json`'s "22.64% recall, 41 false
> negatives". That figure was an artifact: the baseline was generated by a
> `CGO_ENABLED=0` build, which registers no tree-sitter parsers, so every
> AST-based rule matched nothing while the pure-Go secrets/SCA/config scanners
> kept passing. **There was no 41-detection coverage hole.** The corpus now
> scores TP=450 FP=0 FN=0 (100%) on a CGo build.
>
> Two real defects were found while chasing that phantom, both now fixed:
> a shared-rule-index mutation in `engine.Match` that dropped findings at
> random under concurrency (identical scans returned 6–11 of 11 expected
> findings — the unresolved Sprint 25 "non-reproducible anomaly"), and a
> tree-sitter CST lifetime hazard in all 8 IR builders.
>
> The genuine coverage gap is **not** measured by this corpus, which is
> self-authored: fixtures are copied from `testdata/` and expectations are
> written by the rule's own author, so FN=0 is structural. The honest number is
> in `docs/release-notes/QA-REPORT-SPRINT-27.md:306-365`, measured against real
> vulnerable apps: **11/19 missed in damn-vulnerable-golang (58%), 22/40 in
> Vulnerable-Flask-App (55%)**. Prioritise against that, and against the
> Semgrep differential once it exists — not against the list below, which is
> unranked and was never evidence-based.
