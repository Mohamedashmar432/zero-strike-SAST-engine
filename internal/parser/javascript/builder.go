...
func (b *IRBuilder) buildNode(node *sitter.Node, source []byte, parent *ir.IRNode, path string, warnings *[]ir.BuildWarning) *ir.IRNode {
	...
	if node.ChildCount() == 0 {
		...
		// tree-sitter-javascript's string/template_string nodes wrap their
		...
		// argument_literal_matches on crypto.createHash('md5')).
...
// node into a string slice (plain identifiers, defaulted and rest parameters —
